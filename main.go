		...
		*/
		...
		/*
			...
			for _, name := range fieldName {
				if name != "Description" {
					fmt.Print(name)
					str[o] = convertToStringSlice(extractField(tasks, name))

					if MaxString(str[o]) < len(name) { // recuperer le plus grand caracteres pour chaque colonnes
						max[o] = len(name)
					} else {
						max[o] = MaxString(str[o])
					}

					for i := 0; i < max[o]-len(name); i++ {
						fmt.Print(" ")
					}
					fmt.Print(" | ")
					o++
